...
export default function ReleasedItemsPage() {
  ...
  useEffect(() => {
    fetchReleasedParcels();
  }, []);

  const fetchReleasedParcels = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/parcels/released");
      setParcels(response.data);
    } catch (error) {
      console.error("Error fetching released parcels:", error);
    }
  };
  ...
  const updateParcelStatus = async (parcelId, newStatus) => {
    try {
      ...
      fetchReleasedParcels(); // Refresh the list after updating
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-4">Released Items Ready for Shipment</h2>
